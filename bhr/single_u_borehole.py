...
class SingleUBorehole(UTube):
    ...
    def update_beta(self, m_dot: float) -> float:
        """
        ...
        pipe_resist = self.calc_fluid_pipe_resist(m_dot)
        self.pipe_resist = pipe_resist
        beta = self.two_pi_kg * pipe_resist

        return beta

    def calc_direct_coupling_resistance(self, m_dot: float) -> tuple:
        r_a = self.calc_total_internal_bh_resistance(m_dot)
        r_b = self.calc_local_bh_resistance(m_dot)
    ...
    def calc_local_bh_resistance(self, m_dot: float) -> float:
        """
    ...
    def calc_total_internal_bh_resistance(self, m_dot: float) -> float:
        """
    ...
    def calc_grout_resistance(self, m_dot: float) -> float:
        """
        ...
        resist_bh_grout = self.calc_local_bh_resistance(m_dot) - self.pipe_resist / 2.0
        return resist_bh_grout

    def calc_effective_bh_resistance_uhf(self, m_dot: float) -> float:
        """
    ...
    def calc_effective_bh_resistance_ubwt(self, m_dot: float) -> float:
        ...
        r_a = self.calc_total_internal_bh_resistance(m_dot)  # R_a
        r_b = self.calc_local_bh_resistance(m_dot)  # R_b
        r_v = self.bh_length / (m_dot * self.fluid_cp)  # (K/(w/m)) thermal resistance factor
        n = r_v / (r_b * r_a) ** 0.5
