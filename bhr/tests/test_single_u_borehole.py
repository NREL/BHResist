...
class TestSingleUBorehole(TestCase):
    ...
    def test_calc_internal_and_grout_resistance(self):
        ...
        self.assertAlmostEqual(bh.calc_total_internal_bh_resistance(pipe_resist=0.05), 0.32365, delta=tolerance)
        self.assertAlmostEqual(bh.calc_grout_resistance(pipe_resist=0.05), 0.17701, delta=tolerance)

        self.inputs.update({"soil_conductivity": 1.0, "grout_conductivity": 3.6})
        bh = SingleUBorehole(**self.inputs)
        self.assertAlmostEqual(bh.calc_total_internal_bh_resistance(pipe_resist=0.05), 0.17456,
                               delta=tolerance)
        self.assertAlmostEqual(bh.calc_grout_resistance(pipe_resist=0.05), 0.03373, delta=tolerance)

    def test_calc_bh_effective_resistance_uhf(self):
        bh = SingleUBorehole(**self.inputs)
        tolerance = 1e-3
        self.assertAlmostEqual(bh.calc_effective_bh_resistance_uhf(0.5, 20), 0.22615, delta=tolerance)
